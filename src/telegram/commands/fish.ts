...
export async function fishCommand(ctx: Context) {
  await skillingCommand(ctx, 'fishing', 'rod');
}

async function skillingCommand(ctx: Context, skill: 'fishing', toolType: 'rod') {
  ...
  if (player.inCombat) return ctx.reply('You cannot fish while in combat.');
  ...
  if (!resources.length) return ctx.reply('No fishing spots in this area.');
  ...
  if (resourceArg) {
    ...
    if (!resource) return ctx.reply('That fish is not available here.');
  } else {
    const available = resources.filter(r => level >= r.levelRequired);
    resource = available[available.length - 1] || null;
    if (!resource) return ctx.reply('Your fishing level is too low for fish here.');
  }

  if (level < resource.levelRequired) return ctx.reply(`You need fishing level ${resource.levelRequired} to catch ${resource.name}.`);
  ...
  if (quantity === 1) {
    ...
    return ctx.reply(`Caught 1x ${resource.name}. +${totalExp} ${cap(skill)} XP.${msg}`);
  }
  ...
  await player.save();
  await ctx.reply(`Catching ${quantity}x ${resource.name}... (${Math.floor(totalTime/1000)}s)`);
  ...
  setTimeout(async () => {
    try {
      ...
      await p.save();
      const msg = expRes.leveledUp ? ` Level up! ${cap(skill)} is now ${expRes.newLevel}.` : '';
      await ctx.reply(`Fishing complete: ${quantity}x ${resource.name}. +${totalExp} ${cap(skill)} XP.${msg}`);
    } catch (err) {
      console.error('Error completing fishing:', err);
      const p = await Player.findOne({ userId });
      if (p) { p.isSkilling = false; (p as any).currentSkill = null; (p as any).skillingEndTime = null; await p.save(); }
      await ctx.reply('An error occurred while completing fishing.');
    }
  }, totalTime);
}
