...
export async function mineCommand(ctx: Context) {
  await skillingCommand(ctx, 'mining', 'pickaxe');
}

async function skillingCommand(ctx: Context, skill: 'mining', toolType: 'pickaxe') {
  ...
  if (player.inCombat) return ctx.reply('You cannot mine while in combat.');
  ...
  if (!resources.length) return ctx.reply('No mining resources in this area.');
  ...
  if (resourceArg) {
    ...
    if (!resource) return ctx.reply('That resource is not available here.');
  } else {
    // choose the highest level resource the player qualifies for
    const available = resources.filter(r => level >= r.levelRequired);
    resource = available[available.length - 1] || null;
    if (!resource) return ctx.reply('Your mining level is too low for resources here.');
  }

  if (level < resource.levelRequired) return ctx.reply(`You need mining level ${resource.levelRequired} to mine ${resource.name}.`);
  ...
  if (quantity === 1) {
    ...
    return ctx.reply(`Mined 1x ${resource.name}. +${totalExp} ${cap(skill)} XP.${msg}`);
  }
  ...
  await player.save();
  await ctx.reply(`Mining ${quantity}x ${resource.name}... (${Math.floor(totalTime/1000)}s)`);
  ...
  setTimeout(async () => {
    try {
      ...
      await p.save();
      const msg = expRes.leveledUp ? ` Level up! ${cap(skill)} is now ${expRes.newLevel}.` : '';
      await ctx.reply(`Mining complete: ${quantity}x ${resource.name}. +${totalExp} ${cap(skill)} XP.${msg}`);
    } catch (err) {
      ...
      await ctx.reply('An error occurred while completing mining.');
    }
  }, totalTime);
}
